...
await step('PostVacancy - form renders', async () => {
  const body = await page.textContent('body');
  if (!body.includes('Компания') && !body.includes('Ширкат')) throw new Error('PostVacancy form not found');
});
...
await step('ZAKA - open chat widget', async () => {
  // ZAKA button is bottom-right
  const allBtns = await page.$$('button');
  let clicked = false;
  for (const btn of allBtns) {
    const box = await btn.boundingBox().catch(() => null);
    if (box && box.x > 1150 && box.y > 650) {
      await btn.click();
      clicked = true;
      break;
    }
  }
  if (!clicked) throw new Error('ZAKA button not found at bottom-right');
...
await step('Dark mode toggle', async () => {
  const moonBtn = await page.$('button[title]');
  if (moonBtn) {
    await moonBtn.click();
    await page.waitForTimeout(300);
